...
build-win:    ## Build the Windows executable.
	...
	uv run pyinstaller --onefile src/main.py --name adc --add-data "config.yaml;." --hidden-import adc.cli.cli --hidden-import adc.gui.gui
	rm -rf release
	mkdir release
	cp dist/adc.exe release
...
build-macos:    ## Build the macOS executable.
	...
	uv run pyinstaller --windowed --name "TemplateApp" src/main.py --add-data "config.yaml:." --hidden-import adc.cli.cli --hidden-import adc.gui.gui
