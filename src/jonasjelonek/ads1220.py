...
Updated to pass pyright checks 2026 Thomas Müller
...
"""
    ...
    Default pins for communication (pin mode: board)
        ...
        Pin 21  => SPI_MISO (alias: DOUBT on adc)
        Pin 23  => SPI_CLK
        Pin 24  => SPI_CS
...
class ADS1220:
    PIN_DRDY = 17  # DATA READY pin from adc
    ...
    def read_register(self, register):
        opcode = (
            ADS1220.CMD_RREG + (register << 2) + 0x00
        )  # Command opcode + register number (left-shifted by 2) + number of bytes to read minus 1
        # Write opcode + zero-byte to adc
        data = self.spi.xfer([opcode, 0x00])  # pyright: ignore[reportAttributeAccessIssue]
        return data[1]

    def write_register(self, register, value):
        opcode = (
            ADS1220.CMD_WREG + (register << 2) + 0x00
        )  # Command opcode + register number (left-shifted by 2) + number of bytes to write minus 1
        # Write opcode + value to write to adc
        self.spi.xfer([opcode, value])  # pyright: ignore[reportAttributeAccessIssue]
    ...
    def start_conversion(self):
        ...
        self.spi.xfer([opcode])  # pyright: ignore[reportAttributeAccessIssue]
    ...
    def __read_data_rdata(self):
        ...
        data = self.spi.xfer([opcode, 0x00, 0x00, 0x00])  # pyright: ignore[reportAttributeAccessIssue]
    ...
    def next_higher_gain(self):
        if self.current_gain == 1:
            ...
            return self.current_again  # pyright: ignore[reportAttributeAccessIssue]
    ...
    def __read_raw_adc(self):
        if self.conv_mode == 1:
            ...
            self.start_conversion()
            val = int.from_bytes(self.__read_data_drdy(), byteorder="big", signed=True)  # pyright: ignore[reportArgumentType]
    ...
    def config_ADC(self):
        ...
        self.conv_mode = 0  # 0 = single shot mode, 1 = continuous mode
        cfg1 = (
            ADS1220.CFG1_BCS_OFF
            | ADS1220.CFG1_TS_OFF
            # | ADS1220.CFG1_CM_CONTINUOUS
            | ADS1220.CFG1_CM_SINGLE_SHOT
            | ADS1220.CFG1_MODE_NORMAL
            | ADS1220.CFG1_DR_STAGE3
        )
        self.VREF = 5.0  # Set to +2.048V for internal reference
        cfg2 = (
            ADS1220.CFG2_IDAC_OFF
            | ADS1220.CFG2_PSW_AUTO_OPEN
            | ADS1220.CFG2_FIR_NONE
            | ADS1220.CFG2_VREF_EXT_AIN0_AIN3
        )
